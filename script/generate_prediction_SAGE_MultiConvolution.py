        ...
        for filename, df in tqdm(test_df.groupby('filename')):
            ...
            if not os.path.exists(save_file_path):
                with torch.no_grad():
                    ...
                    # adj = torch.ones((codevec_padded_tensor.size(0),codevec_padded_tensor.size(0))).cuda()
                    # adj = torch.randn((codevec_padded_tensor.size(1),codevec_padded_tensor.size(1))).cuda()
