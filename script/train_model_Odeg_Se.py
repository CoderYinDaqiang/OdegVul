...
save_model_dir = '../output/model/OdegVul_Se/'
...
def get_code(df, to_lowercase = False):
...
def train_model(dataset_name):
    ...
    loss_dir = '../output/loss/OdegVul_Se/'
    ...
    _, train_label = get_code2d_and_label(train_df, True)
    _, valid_label = get_code2d_and_label(valid_df, True)


    train_code2d = get_code(train_df, True)
    valid_code2d = get_code(valid_df, True)

    from sklearn.feature_extraction.text import CountVectorizer
    vectorizer = CountVectorizer()
    vectorizer.fit(train_code2d)
    x_train_vec = vectorizer.transform(train_code2d).toarray()
    ...
    for epoch in tqdm(range(current_checkpoint_num,num_epochs+1)):
        ...
        with torch.no_grad():
            ...
            for inputs, labels in valid_dl:
                ...
                output, _, __, ___ = model(inputs, adj)
